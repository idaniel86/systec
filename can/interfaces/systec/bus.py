...
try:
    if sys.platform == "win32":
        from .canlib import USBCanServer

        class Ucan(USBCanServer):
            def __init__(self):
...
class UcanBus(BusABC):
    ...
    def send(self, msg, timeout=None):
        ...
        :raises can.interfaces.sysworxx.exceptions.UsbCanError:
            Error messages, that can occur in the library.

        :raises can.interfaces.sysworxx.exceptions.UsbCanCmdError:
    ...
    def flush_tx_buffer(self):
        ...
        self._ucan.reset_can(self.channel, ResetFlags.RESET_ONLY_TXCHANNEL_BUFF)
